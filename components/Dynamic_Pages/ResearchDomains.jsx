...
const ResearchDomains = ({ post, type, subtype, slug, area_slug }) => {

    const [data, setData] = useState([]);

    const areaname = area_slug;
    const domainname = slug;
    const [searchTerm, setsearchTerm] = useState("");
    const [value] = useDebounce(searchTerm, 1000);

    const [searchType, setsearchType] = useState(areaname);
    const [searchDomain, setsearchDomain] = useState(domainname);
                        ...
                        <div className="col-md-3">
                            <div className="select-wrap mr-auto">
                                ...
                                <select name="" id="" className="form-control topics-section" onClick={(event) => {
                                    ...
                                    {type.map((item) => {
                                        return (<>{item.slug == areaname ? <option selected value={item.slug} >{item.title}</option> : <option value={item.slug} >{item.title}</option>}</>);
                                    })}
                        ...
                        <div className="col-md-3">
                            <div className="select-wrap mr-auto">
                                ...
                                <select name="" id="" className="form-control topics-section" onClick={(event) => {
                                    ...
                                    {searchType ? subtype != undefined && subtype != null ? subtype.map((item) => {
                                        return (
                                            <>
                                                {item.research_area ? searchType === item.research_area.slug ?
                                                    domainname ? domainname === item.slug ? <option selected value={item.slug}>{item.title}</option> : <option value={item.slug}>{item.title}</option> : <option value={item.slug}>{item.title}</option>
                                                    : '' : ''}
                                            </>
                                        );
                                    }) : '' : ''}
                                                ...
                                                <div className="job-post-item bg-white py-3 pl-3 d-block d-md-flex align-items-center" >
                                                    ...
                                                    <div className="col-lg-9 col-md-7 mb-4 mb-md-0 px-0 px-md-2">
                                                        ...
                                                        <div className="text-ellipsis" style={{ color: "#4d4d4d" }}>
                                                            ...
                                                            {/* keywords */}
                                                            {blog.keywords != "" ? <div className="d-flex ml-0 flex-wrap">
                                ...
                                <div className="container">
                                    ...
                                    <Modal show={show} onHide={handleClose} size="md">
                                        <Modal.Header closeButton>
                                            <Modal.Title className="pl-3">Request Your Topic</Modal.Title><AiOutlineClose className="mt-2" onClick={handleClose} style={{ cursor: "pointer" }} /></Modal.Header>
